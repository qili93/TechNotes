# Kimi-K2.6(1T-A32B)：MLA 全量注意力 + INT4 路由专家
...
> 与 Kimi-K3 的关系：K2.6 是 K3 的前代，text 架构直接复用 DeepSeek-V3（vllm `kimi_k2 → DeepseekV3Config`）——全量 MLA，注意力随序列平方增长；K3 才引入 KDA 混合架构把 3/4 层换成线性注意力。权重量化为 **INT4 (pack-quantized, group=32, 对称) 仅路由专家 Linear**，注意力/共享专家/Dense MLP/lm_head/vision 保持 BF16（config `quantization_config.ignore`）
...
> 因果掩码：MLA 全量注意力只算下三角 ≈ seq²/2——K2.6 无稀疏 topk，**平方项不可约**（对比 GLM-5.3 经 DSA top-2048 后 MLA 退化为线性项）
...
- 模型卡片 (1T-A32B, 61 层, 384 选 8+1, ViT 400M) https://modelscope.cn/models/moonshotai/Kimi-K2.6 (`README.md`)
- 模型参数 https://modelscope.cn/models/moonshotai/Kimi-K2.6 (`config.json`: `text_config`, model_type `kimi_k2`)
- INT4 (group=32, 对称) 仅路由专家量化 Kimi-K2.6 `config.json` (`quantization_config`: compressed-tensors pack-quantized, `ignore` 含 self_attn/shared_experts/dense mlp/lm_head/vision)
